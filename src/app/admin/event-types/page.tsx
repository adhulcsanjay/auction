...
export default function EventTypes() {        
    ...
    const tableHead: readonly Column[] = [
        { id: 'eventTypeName', label: 'Name', minWidth: 80 },
        { id: 'eventDescription', label: 'Description', minWidth: 150 },
    ...
    const getData = async () => {
        try {
            setIsLoading(true);
            const response = await axiosInterceptorInstance.get(`v1/eventtypes`);
            if (response?.data) {
    ...
    const filterHandler = (keyword: string) => {
        ...
        const filteredData = dataTemp.filter((ele: any) => 
                            ele.eventTypeName.toLocaleLowerCase().includes(keyword.toLocaleLowerCase()) || 
                            ele.eventDescription.toLocaleLowerCase().includes(keyword.toLocaleLowerCase()));
    ...
    const deleteHandler = async (row: any) => {
        try {
            const response = await axiosInterceptorInstance.delete(`v1/eventtypes?id_EventType=${row.iD_EventType}&createdBy=${userInfo.iD_Users}&modifiedBy=${userInfo.iD_Users}`);
            const result = response.data.result;
        ...
        <Container sx={{ px: '0 !important', maxWidth: 'none !important' }}>
            ...
            <Box sx={{ marginBottom: '16px', marginTop: '16px' }}>
                <Breadcrumbs aria-label="breadcrumb" sx={{ fontSize: '12px', marginBottom: '5px' }}>
                    ...
                    <Typography color="text.primary" sx={{ fontSize: '12px' }}>Event Types</Typography>
                </Breadcrumbs>
                <Typography variant="h5" color="text.primary" sx={{ fontSize: '18px', fontWeight: 500 }}>Event Types</Typography>
            </Box>
            ...
            >
                <DialogTitle id="scroll-dialog-title">Event Type</DialogTitle>
                <IconButton
                    aria-label="close"
            ...
            <ConfirmAlert open={openConfirmAlert} onClose={handleConfirmAlertClose} type={'CONFIRM'} title={'Delete'} message={`Are you sure you want to delete ${selectedRow?.eventTypeName} ?`}></ConfirmAlert>
        </Container>
    )
}
